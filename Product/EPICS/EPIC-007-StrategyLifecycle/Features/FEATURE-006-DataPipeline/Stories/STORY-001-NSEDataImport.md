...
- Parsers: ticker/options/spot suites processed 3,033 option rows and 163,389 spot rows successfully (see Sprint 0 summary).
- Greeks model: 31 automated unit tests covering delta/gamma/theta/vega/rho plus put-call parity.
- Batch validations: ITM→OTM delta monotonicity and theta decay curves verified on 5-sample batch.
- Catalog: smoke test confirmed native `ParquetDataCatalog` reads (no schema drift) after adopting DESIGN-010 pattern.

## Outstanding Actions
- Import ≥1 year of production NSE data (currently running on 10-month sample).
- Cross-check Greeks versus broker (Zerodha Kite) with ±5% tolerance.
- Publish STRAT-001 backtest using native + Greeks sidecar catalogs to fully satisfy final acceptance boxes.
